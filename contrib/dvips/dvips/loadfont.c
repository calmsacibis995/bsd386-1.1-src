        ...
        register fontdesctype *fd ;
{
   ...
   {  
      ...
      for (del=0; del<=RES_TOLERANCE(fd->dpi); del=del>0?-del:-del+1) {
         ...
         (void)sprintf(name, "%s%s.%dpk", d ? d : "", n, fd->dpi + del) ;
         if (pkfile=pksearch(pkpath, name, FOPEN_RBIN_MODE, n,
                             fd->dpi + del, vdpi))
            return(1) ;
      }
   }
   if (d == NULL || *d == 0) {
      ...
      if (dontmakefont == 0 &&
          (pkfile = pksearch(pkpath, name, FOPEN_RBIN_MODE, n, fd->dpi, vdpi)))
               return(1) ;
#ifndef MSDOS
      dontmakefont = 1 ;
#endif
 ...
 */
   {
      ...
      if (lastresortsizes[0] && fd->dpi < 30000) {
         ...
         for (j = i-1; j >= 0; j--) {
            ...
            (void)sprintf(name, "%s%s.%dpk", d ? d : "", n,
                          lastresortsizes[j]) ;
#ifdef FONTLIB
            if ((pkfile=flisearch(n,(halfword)lastresortsizes[j]))
             || (pkfile=pksearch(d, name, FOPEN_RBIN_MODE, n,
