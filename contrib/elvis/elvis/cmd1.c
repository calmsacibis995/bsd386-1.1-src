/*	BSDI $Id: cmd1.c,v 1.4 1993/02/23 19:15:38 polk Exp $	*/
	...
	char	*extra;
{
	...
	rptlabel = "written";
	if (fsync(fd)) {
		msg("fsync failed on %s", extra);
		close(fd);
		return;
	}
	if (close(fd))
		msg("close failed on %s", extra);
}	
	...
	char	*extra;
{
	...
	*scan = '\0';
	...
	for (scan = tmpblk.c; *scan != '\t' && *scan != ' '; scan++)
	{
	}
	*scan++ = '\0';
	...
	char	*extra;
{
	...
	if (func == SIG_IGN)
	{
		/* our parent set us up to ignore SIGTSTP */
		signal(SIGTSTP, func);
		msg("can't suspend, ignoring SIGTSTP");
		return;
	}
