...
#ifndef NO_KILLKEY
char	KILLKEY;	/* line kill key */
#endif
#ifndef	NO_WORDERASEKEY
char	WORDERASEKEY;	/* word erase key */
#endif
#ifndef NO_COLOR
char	normalcolor[16];
char	SOcolor[16];
...
static int		oldquit;	/* ^\ or ^B, the "quit" character */
#   ifdef TIOCSLTC
...
#  ifndef NO_KILLKEY
		KILLKEY = oldtermio.c_cc[VKILL];
#  endif
#  if VWERASE && !NO_WORDERASEKEY
		WORDERASEKEY = oldtermio.c_cc[VWERASE];
#  endif
		newtermio = oldtermio;
		...
		newtermio.c_cc[VQUIT] = _POSIX_VDISABLE;
...
#  endif
#  ifdef VDSUSP
		newtermio.c_cc[VDSUSP] = _POSIX_VDISABLE;
#  endif
#  ifdef TERMIOS
...
#  endif
		...
		ioctl(2, TIOCGETC, (struct sgttyb *) &tbuf);
		oldint = tbuf.t_intrc;
		oldquit = tbuf.t_quitrc;
		tbuf.t_intrc = ctrl('C');	/* always use ^C for interrupts */
		tbuf.t_quitc = \377;		/* disable quit for elvis */
		ioctl(2, TIOCSETC, (struct sgttyb *) &tbuf);

#  ifdef TIOCSLTC
		ioctl(2, TIOCGLTC, &ltbuf);
		oldswitch = ltbuf.t_suspc;
		ltbuf.t_suspc = \377;		/* disable ^Z for elvis */
		olddswitch = ltbuf.t_dsuspc;
		ltbuf.t_dsuspc = \377;		/* disable ^Y for elvis */
		oldquote = ltbuf.t_lnextc;
		ltbuf.t_lnextc = \377;		/* disable ^V for elvis */
		ioctl(2, TIOCSLTC, &ltbuf);
#  endif
	...
	char	*term;
{
	...
	{
	  case -1:
		write(2, "Can't read /usr/share/misc/termcap\n", (unsigned)24);
#if OSK
		write(2, "\l", 1);
#endif
		exit(2);
