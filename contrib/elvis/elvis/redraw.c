	...
	int	inputting;	/* boolean: being called from input() ? */
{
	...
	if (mustredraw)
	{
		...
		{
			/*
			 * If the 'smart' line ends up as the current line
			 * after the delete, smartdrawtext() won't redraw it
			 * even though the screen is now out of date.
			 * The simple thing to do is force a redraw regardless.
			 */
			smartlno = 0L;
