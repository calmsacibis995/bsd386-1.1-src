/*      BSDI $Id: tio.c,v 1.1.1.1 1992/09/17 02:48:49 trent Exp $       */
	...
	int	bsize;	/* size of the buffer */
{
	...
	for (;;)
	{
		...
		if (ch == KILLKEY)
		{
			...
			move(LINES - 1, prompt ? 1 : 0);
			len = 0;
			/* be consistent with \b XXX real vi doesn't do this */
			if (mode == MODE_EX)
			{
				clrtoeol();
			}
...
#define	DO_ERASE() \
	for (ch = widths[len - 1]; ch > 0; --ch) \
		addch('\b')
			...
			/* delete next character, but remember its type */
			DO_ERASE();
			word = isalnum(buf[--len]);

			/* delete characters until last one doesn't match */
			for (; len > 0 &&
			     !isspace(buf[len - 1]) && 
			     isalnum(buf[len - 1]) == word;
			     --len)
				DO_ERASE();
