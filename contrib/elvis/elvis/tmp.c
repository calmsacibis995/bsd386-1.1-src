/*	BSDI $Id: tmp.c,v 1.4 1993/03/23 05:20:24 polk Exp $	*/
	...
	int	bang;		/* forced write? */
{
	...
	/* close the file */
	if (fsync(fd)) {
		msg("fsync failed on %s", filename);
		close(fd);
		return FALSE;
	}
	if (close(fd)) {
		msg("close failed on %s", filename);
		return FALSE;
	}
	...
	int	sig;	/* the deadly signal that we caught */
{
	...
	{
# ifdef SIGHUP
	  case SIGHUP:	why = "-Elvis received a hangup signal";	break;
# endif
# ifndef DEBUG
#  ifdef SIGILL
