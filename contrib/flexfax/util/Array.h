/*	$Header: /bsdi/MASTER/BSDI_OS/contrib/flexfax/util/Array.h,v 1.1.1.1 1994/01/14 23:10:48 torek Exp $
/*
...
#define fxArrayHeader(ARRAY,ITEM)					\
	...
	data = a.raw_copy(); }						\
    ITEM & const operator[](u_int index) const {			\
      fxAssert(index>=0 && index*sizeof(ITEM) < num,			\
	"Invalid Array[] index");					\
...
#define fxIMPLEMENT_ArrayMethods(ARRAY,ITEM)				\
    ...
    ARRAY::~ARRAY() { destroy(); }					\
    const char* ARRAY::className() const { return "ARRAY"; }		\
