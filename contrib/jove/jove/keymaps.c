...
{
	s_mess(ProcFmt);
	BindWMap(mainmap, EOF, (data_obj *) 0);
}
...
void DescCom() { desc_comvar(0); }
void DescVar() { desc_comvar(1); }
...
/* trivial help function -- based on wind.c:WindFind() */
void
Help()
{
	s_mess("Help (A, C, D, V): ");
	switch (CharUpcase(getch())) {
	case 'A':
		ExecCmd((data_obj *) FindCmd(Apropos));
		break;
	case 'C':
		ExecCmd((data_obj *) FindCmd(KeyDesc));
		break;
	case 'D':
		ExecCmd((data_obj *) FindCmd(DescCom));
		break;
	case 'V':
		ExecCmd((data_obj *) FindCmd(DescVar));
		break;
	default:
		complain(
"A: apropos, C: describe-key, D: describe-command, V: describe-variable"
			);
	}
}
