	...
	if (disk_size == 1) {
		...
		if (lseek(fd, where, 0) < 0) {
			...
			exit(1);
					...
					/* read it! */
		if ((n = read(fd, (char *) buf, (unsigned int) len)) != len) {
			if (n < 0) {
				perror("disk_read: read");
				exit(1);
			}
			/* short reads are OK -- just return zeroes */
			bzero(buf+n, len-n);
		}
		return;
	}
		...
		if (i < disk_current || i >= disk_current + disk_size) {
			...
			if (lseek(fd, where, 0) < 0) {
				...
				exit(1);
					...
					/* read it! */
			if ((n = read(fd, (char *) disk_buf, (unsigned int) length)) != length) {
				if (n < 0) {
					perror("disk_read: read");
					exit(1);
				}
				/* short reads are OK -- just return zeroes */
				bzero(disk_buf+n, length-n);
			}
		}
					/* a cache hit... */
