...
/* Support for the new 0314 binary file format.  */

#define A_TEXT_OFFSET(x)        (sizeof (struct exec))

/* TEXT_START isn't quite the same as N_TXTADDR() -- we need to skip
   the header.  We assume a QMAGIC binary by default.  */

#define TEXT_START      (__LDPGSZ + A_TEXT_OFFSET(x))
extern char etext;
#define DATA_START      ((char *) &etext)
