 ...
 * This is also part of the IFF format used by the Amiga.
 */
...
ft_t ft;
{
	...
	/* This is not completely general; other chunk types may be present
	   in AIFF files; but this is all I care about for now. */

	/* COMM chunk */
	if (fread(buf, 1, 4, ft->fp) != 4 || strncmp(buf, "COMM", 4) != 0)
		fail("AIFF header doesn't have 'COMM' chunk");
	chunksize = rblong(ft);
	if (chunksize != 18)
	...
	/* SSND chunk */
	if (fread(buf, 1, 4, ft->fp) != 4 || strncmp(buf, "SSND", 4) != 0)
		fail("AIFF header doesn't have 'SSND' chunk");
	chunksize = rblong(ft);
	offset = rblong(ft);
	blocksize = rblong(ft);
	if (blocksize != 0)
	...
	while (offset > 0) {
		if (getc(ft->fp) == EOF)
			fail("EOF in AIFF file while skipping header");
	}
