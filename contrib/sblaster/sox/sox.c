...
void	gettype(), report(), geteffect(), checkeffect();
...
init() {
	...
	/* init files */
	informat.info.rate      = outformat.info.rate  = 0.0;
...
process() {
	...
	report("Input file: using sample rate %d\n\tsize %s, style %s, %d %s",
...
ft_t ft;
{
	...
	if ((ft->info.rate < 100) || (ft->info.rate > 50000))
		fail("Sampling rate %d for %s file is bogus\n", 
...
"Changing %d input channels to %d output channels with '%s' effect\n",
	...
	if (abs(informat.info.rate - outformat.info.rate) > 200) {
		...
		if (! eff.name) {
			eff.name = rate;
			report(
"Changing sample rate %d to rate %d via noisy 'rate' effect\n",
