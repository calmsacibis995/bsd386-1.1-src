/* @(#)shsdual.c	6.2 1/11/94 00:06:32 */
/*
 * shsdual - shs dual digest code
 *
...
char *shsdual_what="@(#)";	/* #(@) if checked in */
...
/* static declarations */
static void dualData P((BYTE*, UINT, BYTE*, UINT, SHS_INFO*, SHS_INFO*));
static void dualStream P((BYTE*, UINT, FILE*, SHS_INFO*, SHS_INFO*));
static void dualFile P((BYTE*, UINT, char*, SHS_INFO*, SHS_INFO*));
static void dualOutput P((char*, int, SHS_INFO*, SHS_INFO*));
...
dualData(pre_str, pre_len, inString, in_len, even, odd)
    ...
    SHS_INFO *even;		/* even byte digest */
    SHS_INFO *odd;		/* odd byte digest */
{
    ...
    if (len == 1) {
        if (pre_str) {
            shsUpdate(even, (BYTE *)pre_str, 1);
        } else {
            shsUpdate(odd, (BYTE *)inString, 1);
        }
        return;
    }
     ...
     * update both halves
     */
    shsUpdate(even, (BYTE *)even_buf, (len+1)/2);
    shsUpdate(odd, (BYTE *)odd_buf, len/2);
    ...
    SHS_INFO *odd;		/* odd byte digest */
{
	 ...
	 */
	if (epartial) {
	    if (even->datalen == 0 && (elen&(SHS_BLOCKSIZE-1)) == 0) {
		shsfullUpdate(even, (BYTE *)even_buf, elen);
		epartial = 0;
	    } else {
		shsUpdate(even, (BYTE *)even_buf, elen);
	    }
	} else if ((elen&(SHS_BLOCKSIZE-1)) == 0) {
	    shsfullUpdate(even, (BYTE *)even_buf, elen);
	} else {
	    shsUpdate(even, (BYTE *)even_buf, elen);
	    epartial = 1;
	 ...
	 */
	if (opartial) {
	    if (odd->datalen == 0 && (olen&(SHS_BLOCKSIZE-1)) == 0) {
		shsfullUpdate(odd, (BYTE *)odd_buf, olen);
		opartial = 0;
	    } else {
		shsUpdate(odd, (BYTE *)odd_buf, olen);
	    }
	} else if ((olen&(SHS_BLOCKSIZE-1)) == 0) {
	    shsfullUpdate(odd, (BYTE *)odd_buf, olen);
	} else {
	    shsUpdate(odd, (BYTE *)odd_buf, olen);
	    opartial = 1;
    ...
    SHS_INFO *odd;		/* odd byte digest */
{
	 ...
	 */
	if (even->datalen > 0) {
	    shsUpdate(even, (BYTE *)zero, SHS_BLOCKSIZE - even->datalen);
	}
	if (odd->datalen > 0) {
	    shsUpdate(odd, (BYTE *)zero, SHS_BLOCKSIZE - odd->datalen);
	}
    }
...
dualTest()
{
    ...
    SHS_INFO even_dig;		/* even byte digest */
    SHS_INFO odd_dig;		/* odd byte digest */
...
/*
 * dualMain - main driver of shs dual routines
 */
void
