...
; (For large model, follow the instructions given below.)
...
_match_init proc near			; 'proc far' for large model
ifdef SS_NEQ_DS
...
ifdef DYN_ALLOC
  ...
  ifdef SS_NEQ_DS
	...
	ret
	extrn   _exit : near		; 'far' for large model
error:  call    _exit
...
_longest_match  proc near		 ; 'proc far' for large model
        push    bp
        mov     bp,sp
        push    di
	push	si
	push	ds
        ...
        cur_match    equ word ptr [bp+4] ; [bp+6] for large model
