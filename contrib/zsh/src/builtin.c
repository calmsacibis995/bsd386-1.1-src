/*	BSDI	$Id: builtin.c,v 1.2 1994/01/13 23:09:44 polk Exp $	*/
/*
 *
...
struct bincmd builtins[] = {
	...
	{"fc",bin_fc,0,-1,BINF_FCOPTS,BIN_FC,"nlreIRWAdDfE",NULL},
	...
	{"history",bin_fc,0,-1,0,BIN_FC,"nrdDfE","l"},
...
int val;
{
	...
	if (unset(NORCS) && interact) {
		if (isset(APPENDHISTORY)) {
			savehistfile(getsparam("HISTFILE"),0,3);
...
int c;
	...
	while (*args) {
		...
		if (c != -1) {
			if (c == INTERACTIVE)
				zerrnam(nam,"can't change option: %s",args[-1],0);
			else
...
struct hashnode **a;struct hashnode **b;
{
...
int nhash; struct hashnode **hnsorttab, **htp;

	for (nhash = 0, t0 = ht->hsize-1; t0 >= 0; t0--)
		for (hn = ht->nodes[t0]; hn; hn = hn->next)
			nhash++;

	hnsorttab = (struct hashnode **)zalloc (nhash * sizeof(struct hashnode *));
	for (htp = hnsorttab, t0 = ht->hsize-1; t0 >= 0; t0--)
		for (hn = ht->nodes[t0]; hn; hn = hn->next)
			*htp++ = hn;

	qsort((vptr)&hnsorttab[0],nhash,sizeof(struct hashnode *),
			(int (*) DCLPROTO((const void *, const void *)))hnamcmp);

	for (htp = hnsorttab; nhash; nhash--, htp++)
...
char *dest;
	...
	if (!argv[0])
		...
		if (argv[0][1] && argv[0][0] == (isset(PUSHDMINUS) ? '-' : '+')) {
			...
			if (dd < 0 || *end != '\0') {
				zerrnam(nam,"%s: bad directory specification",argv[0],0);
				return NULL;
			}
			...
			if (!strcmp(argv[0],"-")) printdircr(dest = ztrdup(oldpwd));
			else dest = ztrdup(argv[0]);
		}
	} else {
		char *u;
		int len1,len2,len3;
...
int dotsct;
	...
	if (buf2[0] == '/') return (chdir(buf) == -1) ? NULL : ztrdup(buf2);
	if (!dotsct) {
		if (chdir(buf) == -1) return NULL;
	...
	if (s == buf || *buf2) s++;
	strcpy(s,buf2);
	if (chdir(buf) != -1 || chdir(dest) != -1) return ztrdup(buf);
	return NULL;
}
...
long val;
	...
	while (s = *argv++)
		{
		for (lim = -1, t0 = 0; t0 != RLIM_NLIMITS && recs[t0] != NULL; t0++)
...
#ifdef RLIM_INFINITY
	for (t0 = 0; t0 != RLIM_NLIMITS && recs[t0] != NULL; t0++)
		if (t0 == lim || lim == -1)
			{
			printf("%-16s",recs[t0]);
...
struct asgment *asgf = NULL,*asgl = NULL;
	...
	delayrem = !strcmp(nam,"r")
	      && argv[0]
	      &&(argv[1] || !strchr(argv[0],'='));
	if (!delayrem && !(ops['l'] && unset(HISTNOSTORE))) remhist();
	if (ops['R']) {
...
FILE *fout = stdout;
	...
	for (; *args; args++) {
		...
		if (ops['D']) fprintdir(*args,fout);
			...
			int junk;
			fputs(putprompt(*args,&junk,0),fout);
...
char *buf,*bptr;
	...
	while(bptr > buf && isep(bptr[-1])) bptr--;
	*bptr = '\0';
	setsparam(reply,buf);
	if (c == EOF) {
		if (readfd == coprocin) {
...
#endif
#ifdef RLIMIT_MEMLOCK
		case RLIMIT_MEMLOCK: printf("locked-in-memory size (kb) "); t0 /= 1024; break;
#endif
#ifdef RLIMIT_NPROC
		case RLIMIT_NPROC: printf("processes                  "); break;
#endif
#ifdef RLIMIT_OFILE
		case RLIMIT_OFILE: printf("open files                 "); break;
#endif
#ifndef __bsdi__
#ifdef RLIMIT_NOFILE
...
char *usrkeys = NULL;
	...
	for (; *argv && **argv == '-'; argv++)
		...
		case 'k':
			flags |= CC_USRKEYS;
			if ((*argv)[1]) { usrkeys = (*argv)+1; *argv = "\0"; }
			else if (!argv[1]) {
				zerrnam(name,"variable name expected after -k",NULL,0);
				return 1;
			} else { usrkeys = *++argv; *argv = "\0"; }
			break;
