...
int CheckBook (short int side, char *s);
...
{
  et = (time ((long *) 0) - time0) * 100; 
/*  et = (clock ()/CLOCKS_PER_SEC - time0) * 100;*/
  if (et < 0)
    et = 0;
  ETnodes += ZNODES;
  if (iop == 1)
  ...
  do
    {
      ...
      if(max_minutes < 0){max_seconds = -max_minutes;max_minutes=0;}
...
/*  flag.onemove = true; */
  ...
  while (1)
	...
	break;
      inbook = CheckBook (white, white_actual_move);
      if (!inbook)
        {
          flag.force = 0;
  ...
  tmpscore = ScorePosition (white);
	...
	break;
      inbook = CheckBook (black, black_actual_move);
      if (!inbook)
        {
          flag.force = 0;
          SelectMove (computer, 1);
...
#ifdef GDM
int
CheckBook (short int side, char *s)
{
  return false;
}
...
{
/*
 *  Check to see if the move in char *s can be found in the book.
 */
...
unsigned long bhashkey, bhashbd;
short int flag;


bhashkey = hashkey;
bhashbd = hashbd;
if ((gfd = open (binbookfile, O_RDONLY | O_BINARY)) == -1) return false;
read (gfd, &d, sizeof (struct gdxadmin));
currentoffset = (unsigned long) (bhashkey % d.booksize) * sizeof (struct gdxdata) + sizeof (struct gdxadmin);
while (true)
  {
    lseek (gfd, currentoffset, SEEK_SET);
    if (read (gfd, &a, sizeof (struct gdxdata)) == 0) break;
    if (a.bmove == 0) break;
    if (a.hashkey == (unsigned short)(lts(bhashkey)) && a.hashbd == bhashbd)
      {
        m = a.bmove & (~LASTMOVE) & (~BADMOVE);
        flag = 0;
        if (toupper(s[0]) == 'O') flag |= cstlmask;     /*assume castling*/
        algbr (m >> 8, m & 0xFF, flag);
        if (strcmp (s, mvstr[0]) == 0 || strcmp (s, mvstr[1]) == 0 ||
            strcmp (s, mvstr[2]) == 0 || strcmp (s, mvstr[3]) == 0)
        return true;
      }
    currentoffset += sizeof (struct gdxdata);
    if (currentoffset > d.maxoffset)
