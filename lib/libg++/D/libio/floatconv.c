...
struct Bigint {
  ...
  unsigned long x[1<<BIGINT_MINIMUM_K]; /* Actually: x[maxwds] */
};

#define BIGINT_HEADER_SIZE \
  (sizeof(Bigint) - (1<<BIGINT_MINIMUM_K) * sizeof(unsigned long))
...
#endif
{
  ...
  x = 1 << k;
  rv = (Bigint *)
    malloc(BIGINT_HEADER_SIZE + x * sizeof(unsigned long));
  rv->k = k;
  rv->maxwds = x;
  rv->sign = rv->wds = 0;
  rv->on_stack = 0;
...
#ifdef KR_headers
        (result, s, nd0, nd, y9)
	Bigint *result; CONST char *s; int nd0, nd; unsigned long y9;
#else
        (Bigint *result, CONST char *s, int nd0, int nd, unsigned long y9)
#endif
{
  int i, k;
  long x, y;
 ...
 /*     Arguments ndigits, decpt, sign are similar to those
	...
	/* The test is <= so as to allow room for the final '\0'. */
        for(result_k = BIGINT_MINIMUM_K; BIGINT_HEADER_SIZE + j <= i;
                j <<= 1) result_k++;
        result = Brealloc(result, result_k);
        s = s0 = (char *)result;
