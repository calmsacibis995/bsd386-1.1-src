...
hostmatch(char *addr)
{
    ...
    if (isdigit(*addr))
        return (fnmatch(addr, remoteaddr, NULL) != FNM_NOMATCH);
    else
        return (fnmatch(addr, remotehost, FNM_NOCASE) != FNM_NOMATCH);
}
...
acl_setfunctions(void)
{
    ...
    while (!set && getaclentry("compress", &entry)) {
        if (!strcasecmp(ARG0, "yes"))
            l_compress = 1;
        for (which = 1; (which < MAXARGS) && ARG[which]; which++) {
            if (fnmatch(ARG[which], class, NULL) != FNM_NOMATCH) {
...
access_ok(int msgcode)
{
    ...
    if (aclbuf == NULL) {
        syslog(LOG_INFO, "ACCESS DENIED (error reading access file) TO %s [%s]",
