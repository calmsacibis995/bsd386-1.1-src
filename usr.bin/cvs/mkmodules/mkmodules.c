...
#ifndef lint
static char rcsid[] = "$CVSid: @(#)mkmodules.c 1.39 92/03/31 $";
#endif
    ...
    char *argv[];
{
    ...
    /*
     * Finally, keep the "cvsignore" file up-to-date.
     */
    make_tempfile (temp);
	...
	error (0, 0,
	    "a %s file can be used to list per-repository files to ignore",
	    CVSROOTADM_IGNORE);
    (void) unlink_file (temp);
    return (0);
}
