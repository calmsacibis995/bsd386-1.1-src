/*	BSDI $Id: c-typeck.c,v 1.2 1993/10/26 01:08:15 donn Exp $	*/
  ...
  /* Conversions among pointers */
	  ...
	  || comp_target_types (type, rhstype))
	{
	  if (pedantic
	    ...
	    warning_with_arg ("%s between incompatible pointer types", errtype,
	    		      num);
	  else
	    {
	      if (! TREE_READONLY (ttl) && TREE_READONLY (ttr))
		warning_with_arg ("%s of non-const * pointer from const *",
				  errtype, num);
	      if (! TREE_VOLATILE (ttl) && TREE_VOLATILE (ttr))
		warning_with_arg ("%s of non-volatile * pointer from volatile *",
				  errtype, num);
	    }
	}
      else
	warning_with_arg ("%s between incompatible pointer types",
			  errtype, num);
      return convert (type, rhs);
    }
	...
	{
	  warning_with_arg ("%s of pointer from integer lacks a cast",
	  		    errtype, num);
	  return convert (type, rhs);
	}
    ...
    {
      warning_with_arg ("%s of integer from pointer lacks a cast",
      			errtype, num);
      return convert (type, rhs);
    }
