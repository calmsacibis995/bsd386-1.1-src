     ...
     tree idx;
{
  ...
  /* FIXME: This is what breaks vtables on the alpha.  */
  *ptr_to_instptr = build (PLUS_EXPR, TREE_TYPE (*ptr_to_instptr),
			   *ptr_to_instptr,
			   convert (integer_type_node, build_component_ref (aref, delta_identifier, 0, 0)));
     ...
     enum visibility_type visibility;
{
  ...
  if (elem && TREE_VALUE (elem) != (tree)visibility)
	...
	{
	  cp_error_at ("conflicting visibility specifications for method `%D', ignored", TREE_TYPE (fdecl));
	}
      else
	error ("conflicting visibility specifications for field `%s', ignored",
  ...
  /* ARM 11.3: an access declaration may not be used to restrict access
    ...
    cp_error_at ("cannot reduce visibility of public member `%D'", fdecl);
     ...
     tree binfos;
{
  ...
  for (i = 0; i < n_baseclasses; i++)
	...
	b->cant_have_default_ctor = 1;
	  ...
	  && !TYPE_GETS_CONST_INIT_REF (basetype))
     ...
     int warn_anon;
{
  ...
  while (list_of_fieldlists)
      ...
      for (x = TREE_VALUE (list_of_fieldlists); x; x = TREE_CHAIN (x))
	{
	  ...
	  if (DECL_NAME (x) && TREE_CODE (DECL_NAME (x)) == SCOPE_REF)
	    {
	      ...
	      if (last_x)
		TREE_CHAIN (last_x) = TREE_CHAIN (x);
	      /* Make type T see field decl FDECL with visibility VISIBILITY.*/
	  ...
	  /* If this is of reference type, check if it needs an init.  Also
	     do a little ANSI jig if necessary.  */
	  if (TREE_CODE (x) != TYPE_DECL
		...
		ref_sans_init = 1;
		 ...
		 [member].  */
	      if (! TYPE_HAS_CONSTRUCTOR (t) && !TREE_STATIC (x))
		{
		  if (DECL_NAME (x))
	  ...
	  if (current_function_decl && TREE_STATIC (x))
	    cp_error ("field `%D' in local class cannot be static", x);
	  ...
	  if (TREE_CODE (x) == FIELD_DECL || TREE_CODE (x) == VAR_DECL)
		...
		{
		  cp_error ("field `%D' invalidly declared function type",
			      x);
		  TREE_TYPE (x) = build_pointer_type (TREE_TYPE (x));
		}
	      else if (TREE_CODE (TREE_TYPE (x)) == METHOD_TYPE)
		{
		  cp_error ("field `%D' invalidly declared method type", x);
		  TREE_TYPE (x) = build_pointer_type (TREE_TYPE (x));
		}
	      else if (TREE_CODE (TREE_TYPE (x)) == OFFSET_TYPE)
		{
		  cp_error ("field `%D' invalidly declared offset type", x);
		  TREE_TYPE (x) = build_pointer_type (TREE_TYPE (x));
		}
	    }
	  ...
	  if (TREE_CODE (x) == FIELD_DECL)
	      ...
	      if (TREE_TYPE (x) != error_mark_node)
		{
		  ...
		  if (TYPE_LANG_SPECIFIC (TREE_TYPE (x)))
		    {
		      /* It's possible that the type does have a default
			 constructor, *and* have GETS_INIT_REF set, if
			 the class has a non-const copy constructor.  */
		      if (TYPE_HAS_DEFAULT_CONSTRUCTOR (TREE_TYPE (x)))
			needs_default_ctor = 1;
		      if (TYPE_GETS_INIT_REF (TREE_TYPE (x))
			  && !TYPE_GETS_CONST_INIT_REF (TREE_TYPE (x)))
	    ...
	    cp_error ("field `%D' declared static in union", x);
	  ...
	  if (DECL_BIT_FIELD (x))
		...
		{
		  cp_error ("bit-field `%D' has invalid type", x);
		  DECL_INITIAL (x) = NULL;
		}
	      ...
	      if (DECL_INITIAL (x))
		{
		  ...
		  if (width < 0)
		    {
		      DECL_INITIAL (x) = NULL;
		      cp_error ("negative width in bit-field `%D'", x);
		    }
		  else if (width == 0 && DECL_NAME (x) != 0)
		    {
		      DECL_INITIAL (x) = NULL;
		      cp_error ("zero width for bit-field `%D'", x);
		    }
		  else if ((unsigned)width > TYPE_PRECISION (TREE_TYPE (x)))
		    {
		      DECL_INITIAL (x) = NULL;
		      cp_error ("width of `%D' exceeds its type", x);
		    }
		}
	      ...
	      if (code == UNION_TYPE && IS_AGGR_TYPE (type))
		{
		  if (TYPE_NEEDS_CONSTRUCTING (type)
		    ...
		    cp_error ("member `%D' with constructor or destructor not allowed in union", x);
  ...
  /* Create default constructor, if needed.  */
  ...
  if ((TYPE_NEEDS_CONSTRUCTOR (t) || TYPE_HAS_CONSTRUCTOR (t) || needs_ctor)
      && ! TYPE_HAS_INIT_REF (t) && t != EHS_type)
    {
     ...
     int modify;
{
  ...
  *current_class_stack++ = current_class_type;
  ...
  if (previous_class_type != NULL_TREE
	  ...
	  || IDENTIFIER_TEMPLATE (TYPE_IDENTIFIER (previous_class_type)))
...
#else
      pop_class_decls (previous_class_type);
#endif
      while (tags)
	{
