     ...
     int explicit;
{
  ...
  if (TYPE_NEEDS_CONSTRUCTING (type))
      ...
      if (init == NULL_TREE)
	{
	  if (explicit)
	    {
	      error ("incomplete initializer for member `%s' of class `%s' which has no constructor",
	  ...
	  /* member traversal: note it leaves init NULL */
	    ...
	    pedwarn ("uninitialized reference member `%s'",
		     IDENTIFIER_POINTER (name));
	}
     ...
     int immediately;
{
  ...
  /* Initialize all the virtual function table fields that
     dp come from virtual base classes. */
  if (TYPE_USES_VIRTUAL_BASECLASSES (t))
     ...
     char *member_name;
{
  ...
  if (DECL_CONTEXT (field) != type
    ...
    {
      error ("member `%s' comes from base class needing constructor",
	     member_name);
      return 0;
    }
  return 1;
     ...
     tree cname, name, parmlist;
{
  ...
  if (dtor)
	...
	cp_error ("type `%#T' does not have a destructor", cname);
      else
#endif
      cp_error ("cannot call destructor `%T::~%T' without object",
		  IDENTIFIER_TYPE_VALUE (cname), method_name);
      return error_mark_node;
    }
...
/* Tell if TYPE1 is a friend of TYPE2.  */
int
     ...
     tree type1, type2;
{
  ...
  list = CLASSTYPE_FRIEND_CLASSES (TREE_TYPE (TYPE_NAME (type1)));
  while (list)
    {
      if (type2 == TREE_VALUE (list))
	return 1;
...
/* Tell if this function specified by DECL can be a friend of type TYPE.
     ...
     tree type, decl;
{
  ...
  if (ctype && is_friend_type (type, ctype))
    return 1;

  list = DECL_FRIENDLIST (TYPE_NAME (type));
  name = DECL_NAME (decl);
  while (list)
    {
     ...
     int use_global_new;
{
  ...
  if (!has_array && (rval == NULL_TREE || !flag_elide_constructors)
      && TYPE_HAS_CONSTRUCTOR (true_type) && init != void_type_node)
    {
	 ...
	 the visibility of a constructor that matches perfectly.  */
      if (rval != NULL_TREE)
      ...
      if (rval && TYPE_USES_VIRTUAL_BASECLASSES (true_type))
	{
      ...
      newrval = build_method_call (rval, constructor_name_full (true_type),
  ...
  /* It would seem that the above code handles this better than the code
      ...
      if (IS_AGGR_TYPE (type))
	{
	  ...
	  cp_error ("no constructor for type `%T'", type);
	  rval = error_mark_node;
	}
      else
	{
