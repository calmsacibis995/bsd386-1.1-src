     ...
     tree type;
{
  ...
  else error ("cannot allocate an object of type `%T'", type);
...
/* First used: 0 (reserved), Last used: 355. Free: 145. */
      ...
      if (TREE_CODE (init) == CONSTRUCTOR)
	{
	  ...
	  for (field = TYPE_FIELDS (type); field; field = TREE_CHAIN (field))
	    if (TREE_PRIVATE (field) || TREE_PROTECTED (field))
	      {
		cp_error_at ("initializer list construction illegal for `%D'", decl);
		cp_error_at ("due to non-public visibility of member `%D'", field);
	      }
     ...
     tree type, init, *tail;
{
  ...
  if (init && TYPE_PTRMEMFUNC_P (type)
	  ...
	  || integer_zerop (init)))
    {
      init = build_ptrmemfunc (TYPE_PTRMEMFUNC_FN_TYPE (type), init);
    }
     ...
     tree parms;
{
  ...
  if (! IS_AGGR_TYPE (type))
      ...
      if (parms == NULL_TREE)
	{
	  ...
	  cp_error ("cannot cast null list to type `%#T'", type);
	  return error_mark_node;
	}
      if (expr_as_method
