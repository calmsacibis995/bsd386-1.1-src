...
{
  if (is_aggr_typedef (basetype, 1) && datum != error_mark_node)
    {
     ...
     int protect;
{
  ...
  if (! IS_AGGR_TYPE_CODE (code))
    {
      if (code != ERROR_MARK)
	cp_error ("request for member `%D' in something not a class, structure or union", field);
      return error_mark_node;
    }
     ...
     char *errorstring;
{
  ...
  /* `pointer' won't be an error_mark_node if we were given a
    ...
    error ("use of `%s' on pointer to member", errorstring);
     ...
     int convert_p;
{
  ...
  if (convert_p)
	       ...
	       || (IS_AGGR_TYPE (type2) && ! TYPE_PTRMEMFUNC_P (type2)))
	{
	  ...
	  if (try == 0)
	    {
	      cp_error ("no member function `%T::operator %s(%#T)'",
			  types[convert_index],
			  opname_tab[(int) code]
			  ? opname_tab[(int) code] : "<unknown>",
			  types[convert_index ^ 1]);
	      return error_mark_node;
	    }
