...
#ifdef KERNELDEBUG
  if (kernel_debugging) {
#if defined(__i386__) && defined(__bsdi__)
	  static bfd_vma SYSTEM = -1;
	  static struct internal_sys_nlist nl[2] = { { "SYSTEM", }, };

	  if (SYSTEM == -1) {
	  	nlist(abfd->filename, nl);
	  	if (nl[0].n_type == 0)
			error("Kernel symbol `%s' not found.", nl[0].n_name);
		SYSTEM = nl[0].n_value;
	  }
	  obj_textsec (abfd)->vma = SYSTEM + N_TXTADDR(*execp);
	  obj_datasec (abfd)->vma = SYSTEM + N_DATADDR(*execp);
	  obj_bsssec  (abfd)->vma = SYSTEM + N_BSSADDR(*execp);
#else
	  obj_textsec (abfd)->vma = execp->a_entry;
	  obj_datasec (abfd)->vma = execp->a_entry + execp->a_text;
	  obj_bsssec  (abfd)->vma = execp->a_entry + execp->a_text +
				    execp->a_data;
#endif
  } else
#endif
