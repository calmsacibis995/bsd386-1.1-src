...
#else
#define FRAME_CHAIN_VALID(chain, thisframe) \
  (chain != 0 && (!inside_entry_file(FRAME_SAVED_PC (thisframe))))
#endif
...
/* XXX Put comment here about how FRAME_FIND_SAVED_REGS, POP_FRAME,
   and PUSH_DUMMY probably are not the best design. 
   frame_find_saved_regs is used in findvar.c -- we only worry
   about KSA's */
