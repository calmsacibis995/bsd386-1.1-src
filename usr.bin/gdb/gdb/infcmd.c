 ...
 * on signals, etc.  We call clear_breakpoints() to make sure
 * all the breakpoints are out when we let it go.  Otherwise,
 * the program will die when it hits one.  For this to work,
 * it may be necessary for the process to have been
