/*	BSDI $Id: main.c,v 1.2 1994/01/07 21:41:30 donn Exp $	*/
	...
	int batch;
{
	...
	if (corefile[0] == '@')
		...
		if (S_ISCHR(stb.st_mode)) {
			 ...
			 * over a serial link
			 */
			if (major(stb.st_rdev) == MEM_DEVICE) 
				/* kernel memory */
...
#ifdef __bsdi__
  if (corearg == NULL && execarg)
    {
      char *corefile;
      char *slash = strrchr(execarg, '/');
      if (slash)
	++slash;
      else
	slash = execarg;
      corefile = xmalloc(strlen(slash) + 6);
      sprintf(corefile, "core.%s", slash);
      if (access(corefile, R_OK) == 0)
	corearg = corefile;
      else
	free(corefile);
    }
