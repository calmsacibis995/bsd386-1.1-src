  ...
  if (! args)
    {
	  ...
	  && (TYPE_CODE((target = TYPE_TARGET_TYPE(t))) == TYPE_CODE_STRUCT
	      || TYPE_CODE(target) == TYPE_CODE_UNION)
	  && (i = type_field_index(target, name)) >= 0)
	{
	  register int offset;
	  register struct type *f = TYPE_FIELD_TYPE(target, i);

	  offset = TYPE_FIELD_BITPOS(target, i) >> 3;
	  if (TYPE_FIELD_BITSIZE(target, i) == 0)
	    return value_at(f, (CORE_ADDR)(value_as_long(*argp) + offset));
	}
    }
